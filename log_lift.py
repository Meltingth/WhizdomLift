...
# usage: python log_lift.py [PORT] [logfile]
#
# One process per board. COM3 keeps the original log name so the analysis
# tools' defaults still point at lift A; any other port gets its own file.
# All loggers share one STOP_CAPTURE file, so creating it stops every board's
# capture at once.
...
LOG = sys.argv[2] if len(sys.argv) > 2 else os.path.join(_here, _default)
